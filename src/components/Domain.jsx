      ...
      <div data-aos="fade-up" className=" grid grid-cols-2 place-content-center gap-5 w-3/6 max-sm:w-screen ">
        ...
        {domain.map((item, index) => {
          return (
            ...
            >
              <div className="cursor-pointer flex flex-col text-ambers justify-around items-center h-full hover:bg-sky-700 hover:text-white transition ease-in-out duration-300">
                <img  src={item.src} height={48} width={48} alt={item.name}/>
                <h1 className="text-2xl max-sm:text-xl uppercase font-semibold  ">
                  {item.name}
                </h1>
              </div>
            </div>
