...
export default function Navbar() {
    ...
    return (
        <nav className={`select-none fixed z-30 flex p-10 justify-between max-sm:justify-center items-center w-full h-20 bg-zinc-900 font-sans font-semibold text-lg max-sm:text-sm`}>
            <a className="max-sm:hidden text-cyan-50 text-2xl"><img loading="lazy" className="inline-block cursor-pointer" onClick={() => {
                router.reload();
            ...
            <ul className="flex justify-center text-cyan-50">
                ...
                <Link activeClass="active" to="faculty" smooth={true} duration={300} className="max-sm:mr-3 mr-10 cursor-pointer hover:text-sky-700 transition duration-300">Faculty</Link>
